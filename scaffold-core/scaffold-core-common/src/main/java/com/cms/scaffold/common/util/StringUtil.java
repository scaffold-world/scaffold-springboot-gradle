...
public class StringUtil extends StringUtils {
     ...
     * @return
     */
    public static String clearUnderlineCap(String str) {
        char[] charArr = StringUtil.isNull(str).toLowerCase().toCharArray();
        StringBuffer sb = new StringBuffer();
    ...
    public static String replaceTemplate(String template, Map<String, Object> map) {
        //替换固定参数
        template = StringUtils.replace(template, "${noticeTime}", DateUtil.DATEFORMAT_STR_001);

        //替换自定义参数
        for (Map.Entry<String, Object> o : map.entrySet()) {
        	template = StringUtils.replace(template, "${" + o.getKey() + "}", String.valueOf(o.getValue()));
        }
    ...
    public static String replaceTemplateArr(String template, String... params) {
        Pattern p = Pattern.compile("\\$\\{.*?\\}");
        Matcher m = p.matcher(template);
        if (params.length > 0) {
            for (String s : params) {
                m.find();
                template = template.replace(m.group(), s);
            }
        }
        return template;
    }
     ...
     * @return
     */
    public static String replaceLink(String template) {
        if (isBlank(template)) {
            return "";
        }
        Pattern p = Pattern.compile("\\<.*?\\>");
