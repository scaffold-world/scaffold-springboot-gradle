...
public class ResultSetUtils {
    ...
    /**
     * resultSet转换成List<Map> 根据需要转换的字段把resultSet转化为list
     *
     * @param rs
     * @param clude
     * @return
     * @throws Exception
     */
    public static List<Map<String, String>> resultSetToMapClude(ResultSet rs,
                                                                String[] clude) throws Exception {
        ...
        while (rs.next()) {
            ...
            for (int i = 1; i <= colNum; i++) {
                ...
                for (int j = 0; clude != null && j < clude.length; j++) {
                    if (columnName.equalsIgnoreCase(clude[j])) {
                        transFlag = true;
                        break;
                    }
                }
