...
import com.cms.scaffold.route.operate.response.sys.SysDictResp;
...
@Api(tags = "SysDictController", description = "字典管理页面")
    ...
    if (StringUtil.isNotBlank(selectName)) {
      ...
      if (CollectionUtils.isNotEmpty(sysDicts)) {
        if (StringUtil.isNotBlank(q)) {
          sysDicts =
              sysDicts.stream()
                  .filter(sysDict -> StringUtil.contains(sysDict.getName(), q))
