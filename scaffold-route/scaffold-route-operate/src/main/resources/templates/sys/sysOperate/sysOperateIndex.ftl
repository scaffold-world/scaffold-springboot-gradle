...
<script type="text/javascript">
    layui.use(['laydate', 'jquery', 'admin', 'table', 'form'], function () {
        ...
        window.operateTemplate = function (d) {
            var status = d.status;
            if (status === 1) {
                return '<button class=\'layui-btn layui-btn-xs layui-btn-radius\' onclick=\'member_stop(' + d.id + ',2)\'>停用</button>';
            } else {
                return '<button class=\'layui-btn layui-btn-xs layui-btn-radius\' onclick=\'member_stop(' + d.id + ',1)\'>启用</button>';
            }
        };
