...
public interface SysRoleMenuMapper extends BaseMapper<SysRoleMenu> {
    @Select("select * from gjj_sys_role_menu where role_id =#{roleId}")
    List<SysRoleMenu> findByRoleId(Long ruleId);

    @Select("select * from gjj_sys_role_menu where role_id=#{roleId} and menu_id=#{menuId}")
    ...
    @Delete("DELETE from gjj_sys_role_menu where  menu_id in(${uuid}) and role_id=#{roleId};")
    void deleteMenu(@Param("uuid") String uuid, @Param("roleId") Long roleId);

    @Select("select * from gjj_sys_role_menu  where operate_id =#{operateId}")
